...
| `db_set` | Pick the database for a Laravel project: `sqlite`, a built-in (`mysql` / `postgres`), or any installed family alternate (`mariadb`, `postgres-pgvector`, `mysql-5-7`, …); persists to `.lerd.yaml`, rewrites `DB_` keys in `.env`, starts the service, creates the database |
...
| `profiler_toggle` / `profiler_status` / `profiler_clear` | Turn the SPX profiler on/off globally so every PHP-FPM site's HTTP requests are profiled into flame graphs, or clear captured reports |
...
| `db_snapshot` / `db_snapshots` / `db_restore` / `db_snapshot_delete` | Named, restorable database snapshots — create, list, restore (destructive), delete; `all_databases` covers the whole service |
...
- On a **fresh Laravel clone** (DB_CONNECTION=sqlite in `.env`), call `db_set` before `env_setup` to pick a database deliberately — `sqlite`, a built-in (`mysql` / `postgres`), or any installed family alternate (`mariadb`, `postgres-pgvector`, `mysql-5-7`, …). `env_setup` on its own won't switch the database away from sqlite.
